...
export class AdminSuppliersComponent implements OnInit {
  ...
  ngOnInit() { this.load(); }
  load() { this.svc.getAllUsers().subscribe({ next:r=>{ this.loading=false; if(r.success){ this.all=r.data.filter((u:any)=>u.role==='SUPPLIER'); this.apply(); } }, error:()=>this.loading=false }); }
